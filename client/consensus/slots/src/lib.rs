...
pub trait SimpleSlotWorker<B: BlockT> {
		...
		<Self::Proposer as Proposer<B>>::Proposal: Unpin + Send + 'static,
	{
		...
		if let Some(a) = self.backoff_authoring_blocks_strategy() {
			if let Some(ref b) = slot_info.chain_info {
				if a.should_backoff(
					b.chain_head_number,
					b.chain_head_slot,
					b.finalized_number,
	...
	B: BlockT
{
	fn should_backoff(
		...
		if u128::from(slot_now) <= u128::from(chain_head_slot) + interval {
			info!("Backing off authoring blocks due to too many unfinalized blocks");
			true
		} else {
			false
		}
