...
use sc_consensus_slots::{
	...
	SimpleBackoffAuthoringBlocksStrategy, BackoffAuthoringBlocksStrategy,
};
	...
	Error: std::error::Error + Send + From<ConsensusError> + From<I::Error> + 'static,
{
	...
	fn backoff_authoring_blocks_strategy(&self) -> Option<Box<dyn BackoffAuthoringBlocksStrategy<B>>> {
		if let Some(ref a) = self.backoff_authoring_blocks {
	...
	Error: std::error::Error + Send + From<sp_consensus::Error> + From<I::Error> + 'static,
{
	...
	fn on_slot(&mut self, chain_head: B::Header, slot_info: SlotInfo<B>) -> Self::OnSlot {
		...
		let slot_info = {
			...
			let chain_info = if let Ok(chain_head_slot) = find_pre_digest::<B>(&chain_head)
				.map(|digest| digest.slot_number())
			{
				Some(sc_consensus_slots::AppendedChainInfo {
			...
			slot_info.chain_info = chain_info;
			slot_info
		};
